...
# CRAN does not allow the building of baseballstats within the user's library. 
# Vigettes are built locally before being uploaded to CRAN.  CRAN just checks that the code runs. 
# This vignette requires baseball stats. 
# Therefore, suppress chunks on CRAN.  The vignette sans chunks will not be displayed to users.
# Tests locally and on Travis will still check for errors on full script

# This environment variable is set by devtools and also used in testthat. 
